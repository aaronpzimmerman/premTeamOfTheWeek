...
# Merge duplicate names and scores
gw1Revised = gw1Grouped.groupby(['name', 'position', 'team'], as_index=False)['total_points'].sum()
gw1Pos = gw1Revised.loc[:, 'position'] # variable for position sorting below

# helper function 1
def helper1(x):
    x.append({'name': gw1Revised.loc[index, 'name'], 
    'position': gw1Revised.loc[index, 'position'],
    'team': gw1Revised.loc[index, 'team'], 
    'total_points': gw1Revised.loc[index, 'total_points']})
...
# corresponding elements of the gw1Revised DataFrame ('name', 'position', 'team', 'total_points).
# The elements are placed together into one list element of data*Position*. 
for index, position in enumerate(gw1Pos):
    if position == 'FWD':
...
def helper2(y, pos, num1, num2):
    ...
    s1 = sortedPosition.iloc[num1]['total_points']
    s2 = sortedPosition.iloc[num2]['total_points']
    match_count = (sortedPosition['total_points'] == s1).sum()
...
bestForwards = helper2(dataForwards, 'Forwards', 2, 3)
bestMidfielders = helper2(dataMidfielders, 'Midfielders', 4, 5)
bestDefenders = helper2(dataDefenders, 'Defenders', 4, 5)
bestGoalkeepers = helper2(dataGoalkeepers, 'Goalkeepers', 1, 2)
